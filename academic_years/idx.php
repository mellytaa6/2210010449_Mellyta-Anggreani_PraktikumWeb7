...
<div class="main-content">
    <div class="top-bar">
        <h1 class="page-title">Master Data Tahun Ajaran</h1>
        <div class="actions">
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="<?= $row['status'] == 'aktif' ? 'background: rgba(16, 185, 129, 0.1);' : '' ?>">
                    ...
                    <td>
                         <?php if($row['status'] != 'aktif'): ?>
                            <a href="set_active.php?id=<?= $row['id'] ?>" class="btn" style="padding: 5px 10px; font-size: 0.8rem; background: rgba(16, 185, 129, 0.2); color: #10b981; margin-right: 5px;" onclick="return confirm('Set tahun ini sebagai Aktif? Tahun lain akan diset selesai.')">Set Aktif</a>
                        <?php endif; ?>
                        
                        <a href="edit.php?id=<?= $row['id'] ?>" style="color: var(--accent); margin-right: 10px;"><i class="fa-solid fa-pen-to-square"></i></a>
                        <a href="delete.php?id=<?= $row['id'] ?>" style="color: #ef4444;" onclick="return confirm('Hapus tahun ajaran ini?')"><i class="fa-solid fa-trash"></i></a>
                    </td>
                </tr>
                <?php endwhile; ?>
            </tbody>
